...
def importfiletypes():
	types=' \'*.sK1 *.sk1 *.SK1 *.sk *.SK *.ai *.AI *.cmx *.CMX *.cgm *.CGM *.aff *.AFF *.svg *.SVG *.wmf *.WMF *.fig *.FIG|All supported files - *.sK1 *.sk *.ai *.eps *.cmx *.cgm *.aff *.svg *.wmf *.fig \n'
	...
	types=types+' *.cmx *.CMX|CorelDRAW Presentation Exchange files (up to ver. 7.0) - *.cmx \n'
...
def imagefiletypes():
	types=' \'*.png *.PNG *.gif *.GIF *.jpg *.JPG *.jpeg *.JPEG *.tif *.TIF *.tiff *.TIFF *.gif *.GIF *.bmp *.BMP *.pcx *.PCX *.pbm *.PBM *.pgm *.PGM *.ppm *.PPM *.eps *.EPS|All supported files - *.png *.jpg *.tif *.gif *.psd *.bmp *.pcx &etc. \n'
