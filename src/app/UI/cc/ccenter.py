...
class ControlCenter(ModalDialog):
	...
	def build_dlg(self):
		...
		plugpanel = TFrame(toppanel, style='FlatFrame', borderwidth = 1)		
		...
		botpanel = TFrame(self.root, style='FlatFrame', borderwidth=5)		
		botpanel.pack(side = BOTTOM, fill = X, expand=0)
		
		line = TLabel(botpanel, style='HLine')
		line.pack(side = TOP, fill = X, expand=1)
		
		cancel_bt = TButton(botpanel, text=_("Close"), command=self.ok)
