...
from app import _, Publisher, SketchError, _sketch
from app import Blend, CreateRGBColor, MultiGradient
from app.conf.const import DROP_COLOR
...
class GradientView(PyWidget, Publisher):
	...
	def set_gradient(self, gradient):
		...
		for pos, color in gradient:
			self.gradient.append((pos, tuple(color.RGB())))
			...
			for i in range(len(gradient) - 1):
				if gradient[i][0] < pos < gradient[i + 1][0]:
					...
					color = Blend(apply(CreateRGBColor, c2),
									...
									(pos - p1) / (p2 - p1))
					gradient.insert(i + 1, (pos, tuple(color.RGB())))
					self.UpdateWhenIdle()
					break
	...
	def set_handle_color(self):
		if self.context_idx >= 0:
			...
			if color is not None:
				self.gradient[self.context_idx] = (pos, tuple(color.RGB()))
				self.UpdateWhenIdle()
