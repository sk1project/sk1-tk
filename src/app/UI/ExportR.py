...
class CreateRasterParametersDlg(SKModal):
	...
	title = _("Choose Bitmap Parameters")
...
def KGetSaveFilename(title="sK1", filetypes = None, initialdir = '',
								initialfile = ''):
	file = ''
	initialdir =os_utils.gethome()
	file = os.system('kdialog>~/.sK1/tmp.tmp_sK1 -caption \''+title+'\' --getsavefilename \''+initialdir+'\''+filetypes)
	tf=os_utils.gethome()+"/.sK1/tmp.tmp_sK1"
	x=open(tf, "r")
	tmp=x.readline()
	file=tmp[0:len(tmp)-1]
	x.close
	os.remove(tf)
	return file
