# Tamito KAJIYAMA <21 June 2002>

import math
import UserDict

from app.UI.sketchdlg import MessageDialog
from app import CreatePath, Point, Rect
from app import  Bezier, Line # segment types
...
def error(context, message):
	MessageDialog(context.main_window.root, "Path Utils", message)

def get_selection(context):
	if len(context.document.selection) < 2:
		error(context, "Two or more objects must be selected")
		return None
	buffer = []
	for object in context.document.selection.GetObjects():
		if object.is_Rectangle or object.is_Ellipse:
			...
			for path in object.Paths():
				if not path.closed:
					error(context, "Paths in polygon objects must be closed")
					return None
		else:
			error(context,
					"Only rectangles, ellipses and polygons must be selected")
			return None
