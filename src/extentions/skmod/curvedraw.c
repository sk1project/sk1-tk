...
PyObject *
SKCurve_PyCairoDrawTransformed(SKCurveObject * self, PyObject * args)
{
    PyObject * trafo;
    CurveSegment * segment;
    SKCoord lastx, lasty, lastx1, lasty1, lastx2, lasty2;
    PyObject * fill, * line, *rect_or_none;
    PaxGCObject * gc_object;
    SKRectObject * clip_rect = NULL;
    int i;
	...
	{
		SKTrafo_TransformXY(trafo, segment->x, segment->y, &lastx, &lasty);
		cairo_line_to(gc_object->cairo, (double)lastx, (double)lasty);
	}
	else
	{
		SKTrafo_TransformXY(trafo, segment->x1, segment->y1, &lastx1, &lasty1);
		SKTrafo_TransformXY(trafo, segment->x2, segment->y2, &lastx2, &lasty2);
		SKTrafo_TransformXY(trafo, segment->x, segment->y, &lastx, &lasty);
		cairo_curve_to(gc_object->cairo,
					(double)lastx1, (double)lasty1,
					(double)lastx2, (double)lasty2,
...
PyObject *
SKCurve_PyCairoDrawMultipath(PyObject* self, PyObject * args)
{
    PyObject * trafo;
    int i, j;
    PyObject *fill_func, *line_func, *push_clip, *pop_clip, *set_clip;
...
PyObject *
SKCurve_PyCairoFillMultipath(PyObject* self, PyObject * args)
{
    PyObject * trafo;
    int i, j;
    PyObject *fill_func, *line_func, *push_clip, *pop_clip, *set_clip;
