...
#										+ list(aclass.context_commands)
	...
	def KeyPressEvent(self, event):
		...
		try:
			...
			modm = state & const.MetaMask and 'Alt+' or ''
			modc = state & const.ControlMask and 'Ctrl+' or ''
			mods = state & const.ShiftMask and 'Shift+' or ''
			if char:
				mod = modm + modc
			else:
				mod = modm + modc + mods
	...
	def set_handles(self, handles):
		from app.conf.const import HandleLine, Handle_Pixmap, Handle_Caret, \
